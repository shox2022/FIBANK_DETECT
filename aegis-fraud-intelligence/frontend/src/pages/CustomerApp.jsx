  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-lg font-semibold text-slate-950">Latest Decision</h2>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-3">
        ...
        <div>
          ...
          <p className="font-semibold text-slate-950">{result.friction?.label || result.recommendation || "None"}</p>
        </div>
        <div>
          <p className="text-xs uppercase text-slate-500">Transaction status</p>
...
export default function CustomerApp() {
  ...
  function remember(label, data) {
    setResult(data);
    setEvents((current) => [{ label, data, at: new Date().toISOString() }, ...current].slice(0, 6));
  ...
  async function runDemo(action) {
    ...
    const actions = {
      ...
      "germany-vpn": () => api.post("/api/simulate/login", { ...commonLogin, device_hash: "phase5_germany_vpn", ip_address: "93.184.216.34", country: "Germany", city: "Berlin", is_vpn: true, failed_attempts: 3 }),
      "impossible-travel": async () => {
        await api.post("/api/simulate/login", commonLogin);
        return api.post("/api/simulate/login", { ...commonLogin, device_hash: "phase5_impossible_device", ip_address: "93.184.216.35", country: "Germany", city: "Berlin", is_vpn: true, failed_attempts: 3 });
      },
      "token-theft": () => api.post("/api/simulate/token-theft", {
      ...
      "high-value-transfer": () => submitTransfer({ to_account: "AL472091000000009999", amount: 2500, currency: "EUR", recipient_name: "New Beneficiary", recipient_is_new: true }),
      "sql-injection": () => api.post("/api/simulate/security-log", { event_type: "SQL_INJECTION_ATTEMPT", endpoint: "/api/auth/login", ip_address: "91.220.33.44", payload_sample: "' OR '1'='1 --" }),
      "mule-ring": () => api.post("/api/simulate/mule-ring", { mule_account: "AL472091000000009998", amount: 450 })
    };
    await runRequest(actions[action] ? action.replaceAll("-", " ") : "demo", action, actions[action]);
  }
  ...
  async function onTransfer(payload) {
    await runRequest("transfer analysis", "transfer", () => submitTransfer(payload));
  }
  ...
  return (
    <main className="min-h-screen bg-slate-100">
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-6">
        {(toast || error) && (
          <div className={`flex items-center gap-2 rounded-md p-3 text-sm ${error ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}>
        ...
        <section className="grid gap-5 lg:grid-cols-[1fr_0.8fr]">
          <div className="rounded-lg bg-slate-950 p-6 text-white shadow-sm">
            ...
            <p className="text-5xl font-bold">EUR {Number(profile?.balance || 0).toLocaleString()}</p>
            <p className="mt-3 text-sm text-slate-400">{profile?.account_number || "Account pending"}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <TrustMeter score={profile?.trust_score || 0} />
          </div>
        </section>
        <div className="grid gap-6 xl:grid-cols-[1fr_0.95fr]">
          <div className="space-y-6">
            <TransactionSimulator onSubmit={onTransfer} loading={loadingAction === "transfer"} />
            <ResultCard result={result} />
          </div>
          <div className="space-y-6">
            <DemoAttackPanel onRun={runDemo} loadingAction={loadingAction} />
            <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="text-lg font-semibold text-slate-950">Recent Simulation Events</h2>
              <div className="mt-4 space-y-3">
                {!events.length ? <p className="text-sm text-slate-500">Run a demo action to populate this feed.</p> : events.map((event) => (
